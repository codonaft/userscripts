...
// TODO: cumlouder.com, tubeon.com, xtits.xxx, eporner.com, pervclips.com, bigbumfun.com, momvids.com, zbporn.com, ok.xxx / perfectgirls.xxx, tubev.sex, youjizz.com, empflix.com, babestube.com, pornwhite.com, pornomira.net, videosection.com, upornia.com, free.brazzers.com, hdsex.org, gay0day.com, collectionofbestporn.com, pissjapantv.com, pervertslut.com, luxuretv.com, 4kporn.xxx, minuporno.com, prndb.net, familyporn.tv, pornoclown.top / pornoclown.com, pervertium.com, familyporner.com, prostasex.org, analdin.xxx, amateur8.com, fapality.com, jizzex.com, xzeroprice.com, xxdbx.com, xmovies.com, porn.com, en-pornhub.com, pornotube.com, deviants.com, porndr.com, yabuchka.net, zagruz.tv, pornzog.com, trueamaters.com, desiresxl.com, vikiporn.com
...
const init = (args = {}) => {
  ...
  subscribeOnChanges(body, nodeChangeSelector, (node, _observer) => {
    ...
    const adText = node.textContent?.trim?.()?.toLowerCase?.();
    const ad = node.matches('.jw-hardlink-inner') && (adText?.includes?.('full video') || adText?.includes?.('unlock'));
...
const sites = {
  ...
  'pmvtube.com': _ => {
    ...
    init({
      ...
      thumbnailSelector: 'article.thumb-block, div.video-block',
      durationSelector: 'span.duration',
      isUnwantedDuration: text => text.includes(':') && timeToSeconds(text) < MIN_DURATION_MINS * 60,
      onNodeChange: node => {
        if (!validLink(node)) return;
  ...
  'spankbang.com': _ => { // FIXME: consistently set random position
    ...
    init({
      ...
      isVideoUrl: href => href.includes('/video/'),
      hideSelector: 'div[x-data="gifPage"], section.timeline, div.positions-wrapper, span.bg-surface-brand-primary.text-primary.absolute',
      onPlaybackStart: _ => {
